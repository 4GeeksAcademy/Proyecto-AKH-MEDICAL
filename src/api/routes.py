...
from api.models import db, User, Doctor, RoleEnum, TokenBlockedList, MedicalHistory
...
def register():
    ...
    db.session.add(user)
    db.session.commit()
    print(f"User created with email: {email}, role: {role}")  # Log para verificar la creación del usuario
    ...
    if role == RoleEnum.DOCTOR.value:
        ...
        db.session.commit()
        print(f"Doctor created with speciality: {speciality}")  # Log para verificar la creación del doctor
